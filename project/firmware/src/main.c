 ...
 *                Data: D4=PC4, D5=PC5, D6=PC6, D7=PC7  |  Control: EN=PC0, RW=PD5, RS=PD6
...
#if KWS_USE_EEPROM_WEIGHTS
static int8_t weight_row[KWS_INPUT_SIZE];
#endif
...
static void kws_run_inference(const int8_t *in_q, int32_t out_logits[KWS_NUM_CLASSES])
{
    ...
    for (uint8_t i = 0; i < KWS_HIDDEN_SIZE; i++) {
        int32_t acc = kws_eeprom_read_fc1_bias(i);
        if (!kws_eeprom_read_fc1_row(i, weight_row)) {
            hidden_q[i] = 0;
            continue;
        }
        for (uint16_t k = 0; k < KWS_INPUT_SIZE; k++) {
            acc += (int32_t)weight_row[k] * (int32_t)in_q[k];
        }
    ...
    for (uint8_t c = 0; c < KWS_NUM_CLASSES; c++) {
        int32_t acc = kws_eeprom_read_fc2_bias(c);
        if (!kws_eeprom_read_fc2_row(c, weight_row)) {
            out_logits[c] = 0;
            continue;
        }
        for (uint8_t i = 0; i < KWS_HIDDEN_SIZE; i++) {
            acc += (int32_t)weight_row[i] * (int32_t)hidden_q[i];
        }
        out_logits[c] = acc;
    }
#else
